...
from database import VehicleDatabaseManager, VehicleProcessingRecord
...
class DealershipOnboardService:
    def __init__(self):
        ...
        self.standard_env_vars = {
            ...
            "OPENROUTER_API_KEY": "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",
    ...
    def _build_dealership_environment_vars(self, onboard_data: DealershipOnboard, phone_number: str) -> List[Dict[str, str]]:
        ...
        # Generate ENVIRONMENT_ID from dealership name (lowercase, no spaces/special chars)
        environment_id = re.sub(r'[^a-z0-9]', '', onboard_data.dealership_name.lower())
    ...
    def _create_placeholder_store_record(self, environment_id: str, dealership_name: str):
        """Create a placeholder vehicle record so dealership appears in store list immediately"""
        try:
            with self.db_manager.get_session() as session:
                # Check if placeholder already exists
                existing = session.query(VehicleProcessingRecord).filter(
                    VehicleProcessingRecord.environment_id == environment_id,
                    VehicleProcessingRecord.stock_number == "PLACEHOLDER"
                ).first()

                if not existing:
                    placeholder = VehicleProcessingRecord(
                        stock_number="PLACEHOLDER",
                        vin="ONBOARDING",
                        vehicle_name="Onboarding Placeholder",
                    ...
                    session.commit()
                    logger.info(f"Created placeholder store record for {dealership_name} ({environment_id})")
        except Exception as e:
            logger.error(f"Failed to create placeholder store record: {e}")
    ...
    async def onboard_dealership(self, onboard_data: DealershipOnboard) -> Dict[str, Any]:
        ...
        am_payload = {
            ...
            "autoDeploy": "yes",
            "repo": "https://github.com/Alihaider-ai/zero-trust-python",
            "branch": "master",
            "serviceDetails": {
                "env": "docker",
                "region": "oregon",
        ...
        am_service_id = am_result.get("service", {}).get("id")

        # Set environment variables separately
        if am_service_id:
            await self.render_client.set_env_vars(am_service_id, am_env_vars)
        results.append({
            "period": "AM",
        ...
        pm_service_id = pm_result.get("service", {}).get("id")

        # Set environment variables separately
        if pm_service_id:
            await self.render_client.set_env_vars(pm_service_id, pm_env_vars)
        results.append({
            "period": "PM",
        ...
        # Step 6: Create placeholder vehicle record so dealership appears in store list
        environment_id = re.sub(r'[^a-z0-9]', '', onboard_data.dealership_name.lower())
        self._create_placeholder_store_record(environment_id, onboard_data.dealership_name)
